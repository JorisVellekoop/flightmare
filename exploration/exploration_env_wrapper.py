...
class ExplorationVecEnv(VecEnv, ABC):
    #
    def __init__(self, impl):
        ...
        # observation space corresponding to depth image input
        # self._observation_space = spaces.Box(
        #     low=np.zeros(self.img_height * self.img_width),
        #     high=np.ones(self.img_height * self.img_width) * np.Inf,
        #     dtype=np.float32
        # )

        #Observation space with multiple input features
        # self._observation_space = spaces.Dict(
        #     {"odometry": spaces.Box(np.ones(self.obs_dim) * -np.Inf,
        #                             np.ones(self.obs_dim) * np.Inf,
        #                             dtype=np.float64),
        #      "depth_image": spaces.Box(low=0, high=np.Inf,
        ...
        self._depth_img_obs = np.zeros(
            (self.num_envs, self.img_height * self.img_width), dtype=np.float32
        )
        #
    ...
    def getDepthImage(self):
        self.wrapper.getDepthImage(self._depth_img_obs)
        return self._depth_img_obs.reshape((self.img_height, self.img_width))
